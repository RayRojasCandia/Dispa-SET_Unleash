...
config = ds.load_config("../ConfigFiles/ConfigBE_BoundarySector.xlsx")

# # config["SimulationDirectory"]="Simulations/BS_2026"
# # Limit the simulation period (for testing purposes, comment the line to run the whole year)
# config['StartDate'] = (2026, 1, 1, 0, 0, 0)
# config['StopDate'] = (2026, 1, 15, 0, 0, 0)
...
# inputs_MTS, results_MTS = ds.get_sim_results(config['SimulationDirectory'], cache=False, inputs_file='Inputs_MTS.p', results_file='Results_MTS.gdx')
# # #

# Load the simulation results:
inputs,results = ds.get_sim_results(path='../Simulations/BE_BoudarySector_Test',cache=False)
...
inputs_MTS, results_MTS = ds.get_sim_results(path='../Simulations/BE_BoudarySector_Test', cache=False, inputs_file='Inputs_MTS.p',results_file='Results_MTS.gdx')

import pandas as pd
#
#
#
# rng = pd.date_range('2026-01-01', '2026-01-15', freq='H')
...
rng = pd.date_range('2023-06-01', '2023-06-14', freq='H')
for i in config['zones']:
    ds.plot_zone(inputs, results, z=i, rng=rng)
    
rng1 = pd.date_range('2023-01-01', '2023-01-14', freq='D')
